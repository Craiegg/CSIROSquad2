...
var counter = false;
    $(() => {
      ...
      for (var i = 0; i<words.length; i++){
        ...
        var tt = document.createTextNode(word[i].label)
        title.appendChild(tt)
        var paragraph = document.createElement("p")
        paragraph.setAttribute("class", "card-text")
        var pt = document.createTextNode(word[i].definition)
        ...
        selectBtn.setAttribute("wName", word[i].label);
        selectBtn.setAttribute("wDescription", word[i].definition);

        selectBtn.addEventListener("click", function (e) {
          var wName = e.target.getAttribute("wName");
          var wDescription = e.target.getAttribute("wDescription");
          showDialog(wName, wDescription);
       })
        ...
        saveBtn.setAttribute("wName", word[i].label);
        saveBtn.setAttribute("wDescription", word[i].definition);
        saveBtn.onclick = function (e){
          var wName = e.target.getAttribute("wName");
          var wDescription = e.target.getAttribute("wDescription");
          saveData(wName, wDescription);
      ...
      //clearing the existing data from the words array
    ...
    function saveData(wName, wDescription){
      var xmlhttp = new XMLHttpRequest();
      var theUrl = "/words";
        xmlhttp.open("POST", theUrl);
        xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlhttp.send(JSON.stringify({ label :  wName, description: wDescription }));
    }
  ...
  var theUrl = "/words";
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        suggestionArray = JSON.parse(this.responseText)
      }
  };
}
getWords();

function showDialog(wName, wDescription){
  $("#dialog-title").html(wName);
  $("#dialog-content").html(wDescription);
  $("#myModal").modal()
}